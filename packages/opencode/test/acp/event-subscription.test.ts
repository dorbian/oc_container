...
function createFakeAgent(root?: string) {
  const registryPath = root ? path.join(root, "acp-sessions.json") : undefined
  const previousRegistryPath = process.env.OPENCODE_ACP_SESSION_FILE
  if (registryPath) {
    process.env.OPENCODE_ACP_SESSION_FILE = registryPath
  } else {
    delete process.env.OPENCODE_ACP_SESSION_FILE
  }
  ...
  const stop = () => {
    ...
    process.env.OPENCODE_ACP_SESSION_FILE = previousRegistryPath
  }
...
describe("acp.agent event subscription", () => {
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const { agent, controller, updates, stop } = createFakeAgent(tmp.path)
  ...
  test("does not emit user_message_chunk for live prompt parts", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const { agent, controller, sessionUpdates, stop } = createFakeAgent(tmp.path)
        const cwd = "/tmp/opencode-acp-test"
  ...
  test("keeps concurrent sessions isolated when message.part.delta events are interleaved", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const { agent, controller, chunks, stop } = createFakeAgent(tmp.path)
        const cwd = "/tmp/opencode-acp-test"
  ...
  test("does not create additional event subscriptions on repeated loadSession()", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const { agent, calls, stop } = createFakeAgent(tmp.path)
        const cwd = "/tmp/opencode-acp-test"
  ...
  test("permission.asked events are handled and replied", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const { agent, controller, stop, sdk } = createFakeAgent(tmp.path)
  ...
  test("permission prompt on session A does not block message updates for session B", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const { agent, controller, chunks, stop, sdk, connection } = createFakeAgent(tmp.path)
  ...
  test("does not emit duplicate synthetic pending after replayed running tool", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const { agent, controller, sessionUpdates, stop, sdk } = createFakeAgent(tmp.path)
        const cwd = "/tmp/opencode-acp-test"
        const sessionId = await agent.newSession({ cwd, mcpServers: [] } as any).then((x) => x.sessionId)
  ...
  test("restores persisted ACP session metadata across agent instances", async () => {
    await using tmp = await tmpdir()
    await Instance.provide({
      directory: tmp.path,
      fn: async () => {
        const first = createFakeAgent(tmp.path)
        const cwd = "/tmp/opencode-acp-test"
        const sessionId = await first.agent.newSession({ cwd, mcpServers: [] } as any).then((x) => x.sessionId)
        first.stop()

        const second = createFakeAgent(tmp.path)
        let promptArgs: any
        second.sdk.session.prompt = async (params?: any) => {
          promptArgs = params
          return {
            data: {
              info: {
